...
function buildChatChannelId(matchId: string): string {
  return (
    "match_" +
    crypto.createHash("sha256").update(matchId).digest("hex").slice(0, 24)
  );
}
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const chatChannelId = buildChatChannelId(sessionId);

      try {
        await axios.delete(
          `http://chat-service:3007/chat/channel/${chatChannelId}`,
        );
        console.log("[collaboration] deleted chat channel", chatChannelId);
      } catch (err: any) {
        console.error(
          "[collaboration] failed to delete chat channel:",
